...
export class MessageService {
  ...
  getMessage(term: any, type: string, properties?: any): Message {
    ...
    switch (type) {
      ...
      case "target": {
        msg =
          "MATCH (n:Target {uniprot_id: $qParam})-[r:REGULATES]-(b) RETURN n, r, b";
        params = { qParam: term };
        break;
      }
      ...
      case "compound": {
        msg =
          "MATCH (n:Compound {compound: $qParam})-[r]-(b) RETURN n, r, b LIMIT 5";
        params = { qParam: term };
        break;
      }
      ...
      case "counts": {
        switch (properties) {
          case "Target": {
            msg =
              "MATCH (n:Target {uuid: $qParam})-[r]-(b) RETURN DISTINCT labels(b),COUNT(labels(b))";
            break;
          }
          case "Compound": {
            msg =
              "MATCH (n:Compound {uuid: $qParam})-[r]-(b) RETURN DISTINCT labels(b),COUNT(labels(b))";
            break;
          }
          case "Pattern": {
            msg =
              "MATCH (n:Pattern {uuid: $qParam})-[r]-(b) RETURN DISTINCT labels(b),COUNT(labels(b))";
            break;
          }
        }
        params = { qParam: term };
      ...
      case "startNodeSearch": {
        ...
        msg =
          "UNWIND $qParam AS qValue MATCH (n:Target {uniprot_id: qValue}) RETURN n AS data UNION UNWIND $qParam AS qValue MATCH (c:Compound {nostereo_hash: qValue}) RETURN c AS data";
        params = { qParam: term };
        break;
      }
      ...
      case "expand": {
        ...
        switch (properties.target) {
          case "Target": {
            ...
            msg =
              start +
              "{uuid:$qParam}) MATCH (n)-[r]-(b:Target) with {segments:[{start: startNode(r), relationship:r, end: endNode(r)}]} AS ret RETURN ret LIMIT 100";
            break;
          }
          case "Compound": {
            msg =
              start +
              "{uuid:$qParam}) MATCH (n)-[r]-(b:Compound) with {segments:[{start: startNode(r), relationship:r, end: endNode(r)}]} AS ret RETURN ret LIMIT 100";
            break;
          }
          case "Pattern": {
            msg =
              start +
              "{uuid:$qParam}) MATCH (n)-[r]-(b:Pattern) with {segments:[{start: startNode(r), relationship:r, end: endNode(r)}]} AS ret RETURN ret LIMIT 100";
            break;
          }
          case "All": {
            msg =
              "MATCH (n) WHERE n.uuid = $qParam MATCH (n)-[r]-(b) with {segments:[{start: startNode(r), relationship:r, end: endNode(r)}]} AS ret RETURN ret LIMIT 100";
            break;
          }
        }
        params = { qParam: term };
      ...
      case "path": {
        ...
        if (term.end.length > 0) {
          ...
          if (!properties.hasCompound) {
            msg = `MATCH p = (t2)-[w *BFS..${properties.distance} (rel, node | rel.max_confidence_value >= ${properties.confidence})]->(q:Target)
              WHERE t2.uuid IN $start
                AND q.uuid IN $end
                AND t2.uuid <> q.uuid
              RETURN p`;
          } else {
            ...
            msg = `MATCH (c:Compound)-[a:TESTED_ON]-(t1:Target)
              WHERE a.activity < ${properties.activity} AND c.uuid IN $start
              WITH t1, COLLECT(c) AS compounds
              MATCH p1 = (t1)-[*BFS..5 (rel, node | rel.max_confidence_value >= 0.01)]->(q:Target)
              WHERE q.uuid IN $end AND t1.uuid <> q.uuid
              UNWIND compounds AS x
              MATCH p2 = (x)-[:TESTED_ON]-(t1)
              RETURN p1, p2;
              `;
          }
        } else {
          if (!properties.hasCompound) {
            msg = `MATCH p = (t2)-[*BFS (rel, node | rel.max_confidence_value >= ${properties.confidence})]->(q:Target)
              WHERE t2.uuid IN $start
                AND t2.uuid <> q.uuid
              RETURN p`;
          } else {
            ...
            msg = `MATCH (c:Compound)-[a:TESTED_ON]-(t1:Target)
              WHERE a.activity < ${properties.activity} AND c.uuid IN $start
              WITH t1, COLLECT(c) AS compounds
              MATCH p1 = (t1)-[*BFS (rel, node | rel.max_confidence_value >= ${properties.confidence})]->(q:Target)
              WHERE t1.uuid <> q.uuid
              UNWIND compounds AS x
              MATCH p2 = (x)-[:TESTED_ON]-(t1)
              RETURN p1, p2`;
          }
      ...
      case 'prediction': {
        ...
        msg = [
          'MATCH (t:Target {uuid: $qParam})',
          'MATCH (t)<-[r1:POTENT_PATTERN_OF]-(p:Pattern)',
          'MATCH (p)-[r2:PATTERN_OF]->(c:Compound)',
          'OPTIONAL MATCH (c)-[tested:TESTED_ON]->(t)',
          'WITH t,p,c,r1,r2,tested',
          'WHERE tested IS NULL',
          'WITH {segments:[',
          '  {start: startNode(r1), relationship: r1, end: endNode(r1)},',
